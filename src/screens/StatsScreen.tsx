...
import { seededRandom, MONTH_NAMES } from '../utils/helpers';
...
function WeeklyCard({ habit }: { habit: typeof HABITS[number] }) {
  ...
  const weekDone: boolean[] = weekOffset === 0
    ? habit.week
    : Array.from({ length: 7 }, (_, d) => seededRandom(habit.id * 1234 + weekOffset * 7 + d) < habit.rate);
...
function MonthlyCard({ habit }: { habit: typeof HABITS[number] }) {
  ...
  const isDone = (dayNum: number) =>
    seededRandom(habit.id * 1234 + year * 400 + month * 31 + dayNum) < habit.rate;
...
function YearlyCard({ habit, year, onYearChange }: {
  ...
  const isDone = (dayOfYear: number) =>
    seededRandom(habit.id * 1234 + year * 400 + dayOfYear) < habit.rate;
            ...
            {monthWeekStarts.map((startWeek, mIdx) => {
              ...
              return (
                <View key={mIdx} style={{ width: (endWeek - startWeek) * CELL }}>
                  <Text style={styles.monthLabel}>{MONTH_NAMES_SHORT[mIdx]}</Text>
                </View>
              );
            })}
          </View>
